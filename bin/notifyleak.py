...
def getnewleak(date: str) -> List[str] :
    '''
    check if a leak already exists in leaks.json
    '''
    red = redis.Redis(unix_socket_path=get_socket_path('cache'), db=4)
    notify = []
    leaks = json.loads(red.get('leaks')) # type: ignore
    for leak in leaks:
        if leak['discovered'].split()[0] == date :
            notify.append(leak['leak_title'])
    return notify
...
def main() -> None :
    ...
    newleaks = getnewleak(str(date.today() - timedelta(days =1)))
    if not newleaks:
        print('No new leak')
        return
