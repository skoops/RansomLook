#!/usr/bin/env python3
"""
Fix parser syntax errors

This script fixes the syntax errors caused by the context manager conversion.
"""
...
def fix_parser_syntax(content: str) -> str:
    """Fix syntax errors in parser files."""
    
    # Fix the context manager pattern that was incorrectly applied
    # Pattern: with open(...) as file: soup=BeautifulSoup(file,'html.parser')
...
def process_parser_files():
    """Process all parser files to fix syntax errors."""
    ...
    for parser_file in parser_files:
        try:
            ...
            fixed_content = fix_parser_syntax(original_content)
            
            if original_content != fixed_content:
                # Create backup
                backup_file = parser_file.with_suffix('.py.backup2')
                ...
                print(f"✓ Fixed syntax: {parser_file.name}")
                fixed_count += 1
            else:
    ...
    print(f"\nFixed syntax in {fixed_count} out of {len(parser_files)} parser files")
